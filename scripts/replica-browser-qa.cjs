...
async function snap(name) {
  ...
  if (referenceNames.has(name+'.png')) {
...
async function main() {
  ...
  await check('UI-09-全部运动分类',async()=>{
    await nav('today');await id('home-exercises').click();let ids=new Set();for(let i=0;i<5;i++){await id('selection-category').nth(i).click();for(const value of await id('selection-exercise').evaluateAll(es=>es.map(e=>e.getAttribute('data-exercise-id'))))ids.add(value)}assert.equal(ids.size,9);await id('selection-category').first().click();await snap('09-运动选择');await nav('today');await id('home-weekly').click();await snap('19-本周运动路径')
    return '5 个分类可切换，共 9 个运动项目。'
  })
  await check('ARUN-07-设备页面',async()=>{
    await nav('profile');await button(/设备授权/).click();await snap('18-设备与数据来源');await page.locator('.device-platforms uni-button').first().click();assert.match(await page.locator('.device-platforms').innerText(),/今日已同步/);await snap('18-设备与数据来源-同步');await nav('profile');await page.locator('.health-management-grid uni-button').filter({hasText:'健康档案'}).click();await snap('17-健康档案')
  })
  await check('UI-11-助手任务和问答',async()=>{
    await nav('assistant');await snap('11-小喜健康助手');assert.equal(await id('assistant-task-question').count(),4);await id('assistant-task-question').first().click();assert.match(await id('assistant-messages').innerText(),/判断依据/);await input('assistant-query','今天运动胸痛怎么办');await id('assistant-send').click();assert.match(await id('assistant-messages').innerText(),/先停止运动/);await id('assistant-shortcut-devices').click();assert.match(await page.locator('.detail-header').innerText(),/设备/)
  })
  await check('ARUN-13-知识搜索文章视频',async()=>{
    await nav('discover');await snap('12-康复资讯');await input('knowledge-search','不存在的知识abcd');await id('knowledge-empty').waitFor();await input('knowledge-search','');await id('knowledge-featured').click();await snap('13-知识文章');assert.ok((await id('knowledge-body').innerText()).length>80);await id('knowledge-back').click();await id('knowledge-filter').last().click();await id('knowledge-video-entry').first().click();await snap('14-知识视频');const video=page.locator('video').first();await video.evaluate(v=>{v.muted=true;return v.play()});await page.waitForTimeout(500);assert.ok(await video.evaluate(v=>v.currentTime>0));await snap('14-知识视频-播放')
  })
