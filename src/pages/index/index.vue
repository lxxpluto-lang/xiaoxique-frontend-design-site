        ...
        <scroll-view class="page-scroll" scroll-y>
          <view v-if="activeNav === 'today'" class="screen today-screen" data-testid="today-screen">
            <view class="today-hero">
              <view class="today-streak"><text>{{ streak }}</text><text>连续打卡</text><button @tap="openData('checkin')">查看日历 ›</button></view>
              <view class="today-hero-copy"><text>{{ checkInDone ? '今天已完成' : '今天的一小步' }}</text><text>{{ checkInDone ? '状态已沉淀，明天继续' : '完成计划后自动打卡' }}</text></view>
            ...
            <view class="today-task" :class="{ complete: taskCompleted }" data-testid="today-core-task">
              ...
              <view class="task-main"><view class="task-media"><MagpieMotion :label="featuredActivity.title" :video-src="featuredActivity.video" :poster="featuredActivity.poster" :riv-src="featuredActivity.rive.enabled ? featuredActivity.rive.src : ''" :artboard="featuredActivity.rive.artboard" :state-machine="featuredActivity.rive.stateMachine" :fit="featuredActivity.rive.fit" /></view><view class="task-body"><view class="tag-row"><text>{{ taskCompleted ? '已完成' : '待完成' }}</text><text v-if="featuredGame.arSupported">AR互动</text></view><text class="task-title">{{ featuredGame.title }}</text><text class="task-copy">{{ featuredGame.subtitle }}</text></view></view>
