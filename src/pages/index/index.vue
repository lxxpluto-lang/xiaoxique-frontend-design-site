    ...
    <template v-else>
        ...
        <scroll-view class="page-scroll" scroll-y>
          <view v-if="activeNav === 'today'" class="screen today-screen" data-testid="today-screen">
            <view class="today-summary">
              <view class="today-summary__head"><view><text>{{ checkInDone ? '今日计划已完成' : '今日计划待完成' }}</text><text>{{ checkInDone ? '运动记录与打卡已保存' : '完成 1 项核心运动即可打卡' }}</text></view><button @tap="openData('checkin')">打卡日历 ›</button></view>
              <view class="today-summary__numbers"><view><text>{{ weeklyCompletedDays }}<text>/7</text></text><text>本周训练</text></view><view><text>{{ completedMinutes }}<text>分钟</text></text><text>今日运动</text></view><view><text>{{ streak }}<text>天</text></text><text>连续打卡</text></view></view>
            </view>
            <view class="today-task" :class="{ complete: taskCompleted }" data-testid="today-core-task">
              <view class="task-heading"><view><text>{{ mode === 'cardiac' ? '医院今日计划' : '今日运动计划' }}</text><text>{{ planReason }}</text></view><text>{{ taskCompleted ? '1/1' : '0/1' }}</text></view>
              <view class="task-main"><view class="task-media"><AppIcon :src="featuredGame.iconPath" :size="62" color="#0F766E" /></view><view class="task-body"><view class="tag-row"><text>{{ taskCompleted ? '已完成' : '待完成' }}</text><text v-if="featuredGame.arSupported">AR互动</text></view><text class="task-title">{{ featuredGame.title }}</text><text class="task-copy">{{ featuredGame.subtitle }}</text></view></view>
              <view class="task-meta"><view><text>任务</text><text>1项</text></view><view><text>预计</text><text>{{ featuredGame.duration }}</text></view><view><text>打卡奖励</text><text>+{{ checkInDone ? 0 : 5 }}积分</text></view></view>
              <text v-if="planAdjustment" class="plan-adjustment">医生已确认：{{ planAdjustment }}</text>
              <button class="task-primary" data-testid="start-today-plan" @tap="handleTodayPrimary">{{ taskCompleted ? '查看本次解读' : '开始今天的运动' }}</button>
            </view>
            ...
            <view class="section-heading"><view><text>更多运动</text><text>按今天的状态选择</text></view></view>
          ...
          <view v-else-if="activeNav === 'discover'" class="discover-screen" data-testid="discover-screen">
            <CompanionHub v-model:active-tab="socialTab" :streak="streak" :team-joined="teamJoined" :team-state="teamState" :team-members="teamMembers" :team-checked-count="teamCheckedCount" :team-progress="teamProgress" :buddy-state="buddyState" :buddy-day="buddyDay" :check-in-done="checkInDone" :today-key="todayKey()" @open-checkin="openData('checkin')" @create-team="createDemoTeam" @join-team="joinTeamByCode" @join-demo-team="joinDemoTeam" @remind-team="remindTeamMember" @set-buddy-cycle="setBuddyCycle" @connect-buddy="connectDemoBuddy" @remind-buddy="remindBuddy" />
          ...
          <view v-else class="screen profile-screen" data-testid="profile-screen">
            ...
            <view class="profile-section"><text>健康管理</text><view class="service-grid"><button @tap="goDetail('devices')"><view><AppIcon src="/static/icons/magpie-line/device.svg" :size="36" /></view><text>设备授权</text><text>{{ deviceConnected ? '已连接' : '去管理' }}</text></button><button @tap="goDetail('health-archive')"><view><AppIcon src="/static/icons/magpie-line/archive.svg" :size="36" /></view><text>健康档案</text><text>目标与来源</text></button><button @tap="goDetail('reward-store')"><view><AppIcon src="/static/icons/magpie-line/badge.svg" :size="36" /></view><text>积分礼品</text><text>双钱包</text></button></view></view>
      ...
      <view v-else class="detail-frame" :data-detail-view="detailView">
        ...
        <scroll-view class="detail-scroll" scroll-y>
          ...
          <view v-else-if="detailView === 'exercise-category'" class="detail-content"><DetailIntro kicker="运动分类" :title="selectedCategory.title" :copy="selectedCategory.description" /><view class="category-recommend"><view class="category-recommend__icon"><AppIcon :src="categoryRecommendedGame.iconPath" :size="72" color="#0F766E" /></view><view><text>今日推荐</text><text>{{ categoryRecommendedGame.title }}</text><text>{{ categoryRecommendedGame.subtitle }}</text><button @tap="selectExercise(categoryRecommendedGame.id)">开始 {{ categoryRecommendedGame.duration }}</button></view></view><view class="simple-exercise-list"><button v-for="game in categoryGames" :key="game.id" @tap="selectExercise(game.id)"><view><text>{{ game.title }}</text><text>{{ game.feature }}<text v-if="game.arSupported"> · AR互动</text></text></view><text>{{ game.duration }} ›</text></button></view></view>
